...
same config and only uses the sub-structs it needs (`,squash` mapstructure tag).
...
- **Logging**: structured `log/slog`. Use `slog.String("error", err.Error())`
  rather than `slog.Error(err)` (the project uses Go 1.26's slog patterns —
  see existing call sites).
